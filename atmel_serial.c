...
#ifdef LIN_SUPPORT
	unsigned int		usart_mode;	/* Get the usart mode from the dts */
...
#ifdef LIN_SUPPORT
	if (of_property_read_u32(np, "atmel,flexcom-usart-mode", &atmel_port->usart_mode)) 
		atmel_port->usart_mode = 0;
...
gclk_fail:
	...
	test_var = atmel_uart_readl(port, ATMEL_US_MR);
#ifdef LIN_SUPPORT
	if( ( (test_var & 0xF) != 0xAu) && ((test_var & 0xF) != 0xBu))
#endif
	atmel_uart_writel(port, ATMEL_US_MR, mode);
...
//	struct timer_params params;
	...
	ktime_t kt;
	struct kwp_timer_ctx *ctx;
	int idx;
	unsigned int state;
	unsigned int parity_mode; 
	unsigned int test_val;
	unsigned int CR_Register_temp;
	struct tty_struct *tty;
	unsigned int uart_send_data;
	struct tty_port *tport;
	struct ktermios newt, oldt;
	unsigned long flags;
	int iter = 0;
//	int send_data[8] = {0x20,0x10,0x01};
	int send_data[8] = {0xAA, 0x55, 0xF0, 0x0F, 0xAA, 0x55, 0xF0, 0x0F};
	int rec_data[50];
	int send_slave[8] = {0x2C, 0x50, 0x01};
	u8 send_byte;		
//	struct atmel_uart_port *aport = tty->driver_data;


	switch (cmd) {
		case ATMEL_IOCTL_SET_KWP_BAUD:
		case ATMEL_IOCTL_SET_LIN_BAUD:
			if (copy_from_user(&inputvalue , (void __user*) arg ,sizeof(inputvalue) ))
				return -EFAULT;
			if(inputvalue == 0)
				return -EINVAL;
		
			baud_div = 0x00;
			baud_div =  ((port->uartclk) / (16*inputvalue)) ;
	
			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXDIS| ATMEL_US_RXDIS);

			atmel_uart_writel(port, ATMEL_US_BRGR, baud_div);


			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXEN| ATMEL_US_RXEN);

			test_val = atmel_uart_readl(port, ATMEL_US_BRGR);
			return 0;
	
			break;

		case ATMEL_IOCTL_SET_MODE:
			/* 1. Disable RX/TX */
			atmel_uart_writel(port, ATMEL_US_CR, ATMEL_US_RXDIS | ATMEL_US_TXDIS);


			atmel_uart_writel(port, ATMEL_US_CR, ATMEL_US_RSTTX | ATMEL_US_RSTRX);
			
			atmel_uart_writel(port, ATMEL_US_IDR, -1);
			atmel_uart_writel(port, ATMEL_US_CR, (1u << 31)); 			
			atmel_uart_writel(port, ATMEL_US_CR, ATMEL_US_RXFCLR | ATMEL_US_TXFLCLR);
			/* 2. USART Mode Register */
			mr = atmel_uart_readl(port, ATMEL_US_MR);
			mr &= ~(ATMEL_US_USMODE | ATMEL_US_PAR | ATMEL_US_CHRL | ATMEL_US_NBSTOP | ATMEL_US_CHMODE);

			mr |= ATMEL_US_USMODE_LIN_MASTER | ATMEL_US_PAR_NONE | ATMEL_US_CHRL_8 | ATMEL_US_NBSTOP_1 | ATMEL_US_CHMODE_NORMAL;

			atmel_uart_writel(port, ATMEL_US_MR, mr);
		    
			/* 3. LIN Mode Register */
			linmr = 0x710;			
			atmel_uart_writel(port, ATMEL_US_LINMR, linmr);

			/* 4. Re-enable RX/TX */
			atmel_uart_writel(port, ATMEL_US_CR, ATMEL_US_RXEN | ATMEL_US_TXEN);


			test_val = atmel_uart_readl(port, ATMEL_US_IMR);
			printk(KERN_INFO "\nIMR %x\n",test_val);
			
			test_val = atmel_uart_readl(port, ATMEL_US_MR);
			printk(KERN_INFO "\nMR %x\n",test_val);

			test_val = atmel_uart_readl(port, ATMEL_US_LINMR);
			printk(KERN_INFO "\nLINMR %x\n",test_val);
			...
			return 0;
			break;
		case ATMEL_IOCTL_SET_LIN_ID:
			
			 atmel_uart_writel(port, ATMEL_US_IDR, -1);
			
			if(copy_from_user (&linid , (void __user*) arg ,sizeof(linid)) )
                                 return -EFAULT;				
			...
			if(linid == 0x3D)
			{
				...
				atmel_uart_writel(port, ATMEL_US_LINMR, linmr);
				while(atmel_uart_readl(port, ATMEL_US_LINMR) != linmr);
			
				atmel_uart_writel(port, ATMEL_US_LINIR,linid);

				while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_LINID));
				while (iter < 8)
                                {
                                        while (!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_RXRDY));

                                        rec_data[iter] = atmel_uart_readl(port, ATMEL_US_RHR) & 0xFF;
                                        iter++;
                                }
			}
			else if(linid == 0x3C)
			{
         	...
         	//              while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_LINID));
				for(iter = 0 ; iter < 8; iter++)
       	                	{
       	                	      while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_TXRDY));
       	                	      atmel_uart_writel(port, ATMEL_US_THR, send_data[iter]);
			...
			while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_LINTC));

			//local_irq_restore(flags);

			test_val = atmel_uart_readl(port, ATMEL_US_CSR);
			
			if(test_val &(ATMEL_US_LINBE | ATMEL_US_LINISFE | ATMEL_US_LINIPE | ATMEL_US_LINCE | ATMEL_US_LINSNRE | ATMEL_US_LINSTE |  ATMEL_US_LINHTE ))
				printk(KERN_INFO "LIN CSR Error %x\t",test_val);
			...
			if(linid == 0x3D)
			{
				for (iter= 0; iter < 8; iter++)
                	        {
        				printk(KERN_INFO "Response Data: %x\t",rec_data[iter]);
                        	}
			}
			return 0;

			break;	
		case ATMEL_IOCTL_ENABLE_LIN_SEND_DATA:

			for(iter = 0 ; iter < 8; iter++)
                        {
				while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_TXRDY));
				atmel_uart_writel(port, ATMEL_US_THR, send_data[iter]);
//				printk(KERN_INFO "TX Data %d\t",send_data[iter]);
			}
			return 0;
			
			break;
		case ATMEL_IOCTL_CHECK_SLAVE_MODE:
			...
			mr &= ~(ATMEL_US_USMODE | ATMEL_US_PAR | ATMEL_US_CHRL | ATMEL_US_NBSTOP | ATMEL_US_CHMODE);
                        ...
                        mr |= ATMEL_US_USMODE_LIN_SLAVE| ATMEL_US_PAR_NONE | ATMEL_US_CHRL_8 | ATMEL_US_NBSTOP_1 | ATMEL_US_CHMODE_NORMAL;
			...
			break;			
		case ATMEL_IOCTL_CHECK_SLAVE_RESPONSE:
			atmel_uart_writel(port, ATMEL_US_IDR, -1);

			while(!(atmel_uart_readl(port, ATMEL_US_CSR) & 0x4000u));
			atmel_uart_writel(port, ATMEL_US_CR, ATMEL_US_RSTSTA);
			test_val = atmel_uart_readl(port, ATMEL_US_LINIR);
			if(test_val == 0x3C)
			{
				linmr = 0x711;
	                        atmel_uart_writel(port, ATMEL_US_LINMR, linmr);
			...
			//	local_irq_restore(flags);
				...
				for (iter= 0; iter < 8; iter++)
				{
					printk(KERN_INFO "Slave Data: %x\t",rec_data[iter]);
				}
				printk(KERN_INFO "\nExit\n");				

			}
			else if(test_val == 0x3D)
			{
				linmr = 0x710;
                                atmel_uart_writel(port, ATMEL_US_LINMR, linmr);
				...
				for(iter = 0 ; iter < 8; iter++)
                                {
					while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_TXRDY));
                                        atmel_uart_writel(port, ATMEL_US_THR, send_slave[iter]);
                                }
				while(!(atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_LINTC));
			...
			return 0;		
			break;
		case ATMEL_IOCTL_CHECK_RX_READY:
			...
			return 0;
			break;
		case ATMEL_IOCTL_ENABLE_KWP_TX_RX:

			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXEN| ATMEL_US_RXEN);
			atmel_uart_writel(port, ATMEL_US_IER , ATMEL_US_TXRDY| ATMEL_US_RXRDY );
			printk(KERN_INFO "IOCTL TO ENABLE TX AND RX\n");
			return 0;
			break;

		case ATMEL_IOCTL_RAW_mode:

			/*
			 dev_info(port->dev, "CSR now 0x%08x IER 0x%08x IDR 0x%08x\n",
        		 atmel_uart_readl(port, ATMEL_US_CSR),
        		 atmel_uart_readl(port, ATMEL_US_IER),
        		 atmel_uart_readl(port, ATMEL_US_IDR));
			*/
			//dev_info(port->dev, "IER after enable = 0x%08x\n", atmel_uart_readl(port, ATMEL_US_IER));

			while (atmel_uart_readl(port, ATMEL_US_CSR) & ATMEL_US_RXRDY)
   				 (void)atmel_uart_readl(port, ATMEL_US_RHR);
			...
			/*
		        * Update the tty’s visible termios and program the HW.
			* Take the port lock to serialize with ISR paths that may look at
			* termios/port state.
			*/
			spin_lock_irqsave(&port->lock, flags);
			tty->termios = newt;
			...
			/*
			* Call the normal driver termios path so serial_core timing/accounting
			* is updated and HW registers are programmed. We’re in the same file,
			* so we can call atmel_set_termios() directly.
			*/
			atmel_set_termios(port, &newt, &oldt);

			port->ops->flush_buffer(port);

			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXDIS| ATMEL_US_RXDIS);				
			atmel_uart_writel(port, ATMEL_US_IDR , ATMEL_US_TXRDY| ATMEL_US_RXRDY | ATMEL_US_ENDTX);
			return 0;
			break;			


		case ATMEL_IOCTL_SET_PARITY:
			if (copy_from_user (&inputvalue , (void __user*) arg ,sizeof(inputvalue)) )
                                  return -EFAULT;
			parity_mode = atmel_uart_readl(port , ATMEL_US_MR);
			parity_mode &= ~ATMEL_US_PAR;

			switch (inputvalue) {
				case ATMEL_PARITY_NONE:
					parity_mode |= ATMEL_US_PAR_NONE;
					break;

				case ATMEL_PARITY_EVEN:
                                         parity_mode |= ATMEL_US_PAR_EVEN;
                                         break;
				case ATMEL_PARITY_ODD:
                                         parity_mode |= ATMEL_US_PAR_ODD;
                                         break;

				case ATMEL_PARITY_SPACE:
                                         parity_mode |= ATMEL_US_PAR_SPACE;
                                         break;

				case ATMEL_PARITY_MARK:
                                         parity_mode |= ATMEL_US_PAR_MARK;
                                         break;
			...
			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXDIS| ATMEL_US_RXDIS);
			atmel_uart_writel(port, ATMEL_US_MR,  parity_mode);
			atmel_uart_writel(port, ATMEL_US_CR,  ATMEL_US_TXEN| ATMEL_US_RXEN);
			return 0;
			break;
...
#ifdef LIN_SUPPORT
	if (atmel_port->usart_mode == 4) {
		/* put usart controller into LIN master mode */
		atmel_uart_writel(&atmel_port->uart, ATMEL_US_MR,(ATMEL_US_USMODE_LIN_MASTER| ATMEL_US_PAR_ODD | ATMEL_US_CHRL_8 | ATMEL_US_NBSTOP_1 | ATMEL_US_CHMODE_NORMAL));
		linmr = 0x800;
                atmel_uart_writel(&atmel_port->uart, ATMEL_US_LINMR, linmr);


	}
	else if(atmel_port->usart_mode == 5)
	{
		atmel_uart_writel(&atmel_port->uart, ATMEL_US_MR,(ATMEL_US_USMODE_LIN_SLAVE| ATMEL_US_PAR_ODD | ATMEL_US_CHRL_8 | ATMEL_US_NBSTOP_1 | ATMEL_US_CHMODE_NORMAL));
		linmr = 0x801;
                atmel_uart_writel(&atmel_port->uart, ATMEL_US_LINMR, linmr);
